...
all_methods = methods_fm + list(set(methods_reduction + methods_expansion))
...
for folder in folders:
    ...
    for path in filepaths:
        ...
        if dataset == 'liverpool':
            reduction_dims = [8]
            expansion_dims = [12, 16, 20, 32, 64, 96, 128]
        else:
            reduction_dims = [8, 12, 16, 20]
            expansion_dims = [32, 64, 96, 128]

        df['inferred_task'] = df['dimension'].apply(
            lambda d: 'reduction' if d in reduction_dims else
                      'expansion' if d in expansion_dims else 'unknown'
        )
        df = df[df['inferred_task'] != 'unknown']
...
for task in folders:
    ...
    # Reorder index: first FMs, then traditional methods
    ordered_methods = methods_fm + [m for m in pivot.index if m not in methods_fm]
...
for task in folders:
    for metric in all_metrics[task]:
        ...
        for method in norm_df['method'].unique():
            ...
            plt.xlim(min(dims) - 1, max(dims) + 1)
