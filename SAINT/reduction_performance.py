...
for dataset in datasets:
    ...
    for dim in dimensions:
        ...
        if not valid_pairs:
            print(f"⚠️ No valid indices found for {method} {dim}D in {dataset}. Skipping.")
            continue
        ...
        # Ensure no NaNs
        print(f"📊 Debug {dim}D - Checking for NaNs")
        print(f"    → NaNs in X_ref: {np.isnan(X_ref).sum()} out of shape {X_ref.shape}")
        print(f"    → NaNs in X_emb: {np.isnan(X_emb).sum()} out of shape {X_emb.shape}")
    ...
    df_results.to_csv(os.path.join(BASE_DIR, f'metrics_{method.lower()}_{dataset.lower()}.csv'), index=False)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(BASE_DIR, f'radar_{method.lower()}_{dataset.lower()}.png'))
    plt.close()
