...
class TabAttention(nn.Module):
    ...
    def get_embeddings(self, x_categ, x_cont, x_categ_enc, x_cont_enc):
        tokens = torch.cat([x_categ_enc, x_cont_enc], dim=1)  # [B, N, dim]
        tokens = self.transformer(tokens)                     # Transformer output
        return tokens.mean(dim=1)                             # Mean pooling → [B, dim]
