...
embedding_output_dir = os.path.join(BASE_DIR, 'CBC', 'embeddings_tabtransformer')
...
for embed_dim in embedding_sizes:
    ...
    model = TabTransformer(
        ...
        dim=embed_dim,
        dim_out=embed_dim,  # Embedding dimension output
        depth=3,
        heads=8,
        attn_dropout=0.2,
        ff_dropout=0.1
    ...
    for epoch in range(50):
        ...
        for batch in train_loader:
            ...
            outputs = model(x_cat, x_num)  # Direct outputs
            loss = criterion(outputs, torch.zeros_like(outputs))  # Force embeddings to stabilize
