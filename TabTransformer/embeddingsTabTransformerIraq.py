...
# All columns are continuous numerical features
...
# Dummy categorical input (TabTransformer API requirement)
...
for embed_dim in embedding_sizes:
    ...
    model = TabTransformer(
        categories=[1],  # Dummy categorical input
        num_continuous=len(feature_cols),
        dim=embed_dim,
        dim_out=len(feature_cols),  # Predict same # of inputs
        depth=3,
        heads=8,
        attn_dropout=0.2,
        ff_dropout=0.1
