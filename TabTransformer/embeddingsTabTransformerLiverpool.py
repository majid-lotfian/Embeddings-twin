...
# Extract and drop ID column
ids = data['id'].values
data = data.drop(columns=['id'])

# Feature categorization
bin_cols = ['sex']  # Already 0/1, no encoding needed
num_cols = [col for col in data.columns if col not in bin_cols]
...
for embed_dim in embedding_sizes:
    ...
    model = TabTransformer(
        categories=[1],  # dummy categorical
        num_continuous=len(feature_cols),
        dim=embed_dim,
        dim_out=len(feature_cols),  # reconstruct input
        depth=3,
        heads=8,
        attn_dropout=0.2,
        ff_dropout=0.1
