...
dim_map = {
    'CBC': [8, 12, 16, 20, 32, 64, 96, 128],
    'Covid19': [8, 12, 16, 20, 32, 64, 96, 128],
    'Iraq': [8, 12, 16, 20, 32, 64, 96, 128],
    'Liverpool': [8, 12, 16, 20, 32, 64, 96, 128]
}
method = 'TabNet'  # Change this to the FM name you're evaluating
...
for dataset in dataset_list:
    ...
    for dim in dim_map[dataset]:
        ...
        # Fix for 3D embeddings (e.g., shape: (2, N, D)), select the first slice
        if embeddings.ndim == 3:
            print(f"⚠️ Detected 3D embeddings with shape {embeddings.shape}. Taking first slice.")
            embeddings = embeddings[0]  # or use embeddings.mean(axis=0) if appropriate
