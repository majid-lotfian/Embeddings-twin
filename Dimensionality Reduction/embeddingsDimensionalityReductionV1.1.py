...
embedding_output_dir = os.path.join(BASE_DIR, 'embeddings_classical_reduction', 'CBC', 'v1.1')
...
class VariationalAutoencoder(nn.Module):
    def __init__(self, input_dim, latent_dim):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.Linear(64, 128),
            nn.ReLU(),
            nn.Linear(128, input_dim)  # ⚠️ No activation here (allow negative outputs)
        )
...
def vae_loss(reconstructed_x, x, mu, logvar, verbose=False):
    recon_loss = nn.functional.mse_loss(reconstructed_x, x, reduction='mean')
    kl_loss = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp()) / x.size(0)
    if verbose:
        print(f"    ↳ Recon: {recon_loss.item():.6f} | KL: {kl_loss.item():.6f}")
...
for dim in reduction_dims:
    ...
    print(f"    ✅ PCA variance explained: {variance_ratio:.4f}")
    ...
    # Open a log file for this dimension
    ...
    with open(vae_log_path, 'w') as log_file:
        ...
        for epoch in range(50):
            ...
            for batch in loader:
                ...
                recon_loss = nn.functional.mse_loss(reconstructed, x_batch, reduction='mean')
